...
load_dotenv("/Workspace/Shared/neural_hydrology/.env")

_env_root = os.environ.get("NEURAL_HYDROLOGY_ROOT", "").strip()
if not _env_root:
    raise RuntimeError(
        "NEURAL_HYDROLOGY_ROOT must be set (environment or .env)"
    )
_REPO_ROOT = Path(_env_root).expanduser().resolve()
